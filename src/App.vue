...
const getSalsa = async () => {
  ...
  const salsa = await fetch(`https://manga.newtyf.com/download-manga-pdf?atomicNumber=${atomicNumber.value}`);
...
<template>
  ...
  <form @submit.prevent="" class="atomicNumber">
    ...
    <a v-if="mostrarSalsa" :href="`https://manga.newtyf.com/${res}`" target="_blank">Listo aqui esta tu manga</a>
  </form>
</template>
